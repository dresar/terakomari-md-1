...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  const anu = await (await fetch(`https://tenor.googleapis.com/v2/featured?key=AIzaSy_SANITIZED_KEY_PROTECTED&contentfilter=high&media_filter=png_transparent&component=proactive&collection=emoji_kitchen_v5&q=${encodeURIComponent(emojis.join('_'))}`)).json();
